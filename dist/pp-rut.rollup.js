(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('pp-is')) :
	typeof define === 'function' && define.amd ? define(['pp-is'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.ppRut = factory(global.ppIs));
})(this, (function (ppIs) { 'use strict';
	...
	ppRut = function(rut="0",digitV=undefined){
		...
		self.digit = 
	    ppIs.isString(digitV) ? 
	    	dClean(digitV.toLowerCase()) : 
	    	isInt(digitV) ? 
	    	numberToDigitV(digitV) : "";
		
		self.body = ppIs.isString(rut) ? ((r,d)=>{

			const tmpSplit = r.split(/[\s,-]{1}(?=[^\s,-]*$)/g);

			if( !ppIs.isUndefined(tmpSplit.at(1)) && self.digit === "" ){			
		...
		self.checkDigit=(digit=undefined)=>self.getDigit() === (ppIs.isUndefined(digit) ? self.digit : (isInt(digit) ? numberToDigitV(digit) : digit ) );	
		self.getDigit=()=>{		
